...
function build(directory, config, parameters, level, seed)
	...
	if archetypeConfig.animationCustom then
		util.mergeTable(config.animationCustom or {}, archetypeConfig.animationCustom)
	end
...
--[[	if archetypeConfig.partLayerOrder then
	...
	if not config.inventoryIcon and config.animationParts then
		...
		local parts = archetypeConfig.partLayerOrder or {}
...
function correctAbility(config, primaryAbilityMultipliers)
	...
	correctedAbility.inaccuracy = correctedAbility.projectileCount > 1 and (correctedAbility.inaccuracy + 0.01 * (1 + (correctedAbility.projectileCount - 1) * 0.3)) or correctedAbility.inaccuracy

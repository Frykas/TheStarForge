...
function init()
  ...
  self.timer = 0
  self.minDistanceToGround = 7
  self.maxDistanceToCeiling = 3
  self.hoverCorrectionSpeed = 10
  self.hoverCycleLength = 1
  self.hoverCycleDuration = 0.75
  self.stablePositionFound = false
  
  self.stopDistance = 2
  self.teleportDistance = 50
  self.returnTime = 4
  self.returnTimer = self.returnTime
  
  self.parentEntity = config.getParameter("parentEntity")
  self.parentEntityUUID = world.entityUniqueId(self.parentEntity)
  self.pathing = {}
...
function update(dt)
  ...
  --world.debugText("GROUND NEAR? : %s", closeToGround, vec2.add(mcontroller.position(), {0,1}), "yellow")
  --world.debugText("CEILING NEAR? : %s", closeToGround, vec2.add(mcontroller.position(), {0,-1}), "yellow")
  ...
  if world.findUniqueEntity(self.parentEntityUUID) then
    local previousPosition = mcontroller.position()
    local position = world.entityMouthPosition(self.parentEntity)
	local targetPosition = vec2.add(position, {-1 * mcontroller.facingDirection(), 2})
	
    --Speed up the drone, the further away from its target it is
    local params = config.getParameter("baseMovementParameters" , {})
	
	--Calculate the speed
	local movementFactor = world.magnitude(mcontroller.position(), targetPosition)
	local targetSpeed = math.sqrt(movementFactor * movementFactor * movementFactor) * 2 + movementFactor * 5 + (params.speed or 70)
	local accelerationFactor = (world.magnitude(mcontroller.velocity()) / targetSpeed) + 0.5
	local finalSpeed = targetSpeed * accelerationFactor * 2
	
    params.flySpeed = finalSpeed
    params.airForce = finalSpeed
    params.liquidForce = finalSpeed
    mcontroller.controlParameters(params)
	
	local obstacle = world.lineCollision(mcontroller.position(), targetPosition)
	if obstacle or (world.magnitude(targetPosition, entity.position()) > self.teleportDistance) then
	  self.returnTimer = math.max(0, self.returnTimer - dt)
	else
	  self.returnTimer = self.returnTime
	end
	
	if self.returnTimer == 0 then
	  teleportToPlayer(previousPosition, targetPosition)
	elseif world.magnitude(targetPosition, entity.position()) > self.stopDistance then
